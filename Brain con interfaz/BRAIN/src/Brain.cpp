...
<<<<<<< Updated upstream:BRAIN/src/Brain.cpp
	int i = random_number(0, 6);
=======
	int i = random_number(0, 7);
>>>>>>> Stashed changes:Brain con interfaz/BRAIN/src/Brain.cpp
...
=======
		move(entities, (i==7)? 7 : i+7); //Si se escoge a la Casa, no se le suma 7
	//Para aumentar el termómetro Emocional
	else if(is_the_first_smaller(Emotional, Biologic, Cultural, 3))
		move(entities, (i==21)? 7 : i+14);	//Si se escoge la casa, no se le suma 14
	
	else if(Biologic >= 6 and Cultural >= 6 and Emotional >= 6){
>>>>>>> Stashed changes:Brain con interfaz/BRAIN/src/Brain.cpp
...
void BRAIN::move(vector<Entity> &entities, int i){
	...
	ofstream file;
	file.open("places.txt", ios::out);
	file << entities[i].get_x() << entities[i].get_y();
	file.close();
}